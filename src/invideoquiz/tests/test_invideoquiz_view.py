...
class TestStudentViewConfig:
    ...
    def test_config_escapes_script_breakout_in_timemap_value(self):
        malicious_id = '</script><script>alert(1)</script>'
        block = _make_block(
            video_id='video-xyz',
            timemap=json.dumps({'1:30': malicious_id}),
            jump_back='{}',
        )
        fragment = block.student_view()
        javascript = self._config_javascript(fragment)
        assert malicious_id not in javascript
