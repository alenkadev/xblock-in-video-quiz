...
def _json_for_inline_script(value):
    """
    Serialize value as JSON safe to embed inline inside a <script> tag.

    json.dumps does not escape "</", so an author-controlled string containing
    that sequence (e.g. a problem id used as a timemap/jump-back key) could
    prematurely close the script tag and inject markup into the LMS.
    """
    return (
        json.dumps(value)
        .replace('<', '\\u003c')
...
class InVideoQuizXBlock(StudioEditableXBlockMixin, XBlock):
    ...
    def _build_quiz_schedule(self, context=None, sibling_names=None):
        ...
        entries = []
        for time_key in sorted(timemap.keys(), key=lambda key: (
                isinstance(time_sort_key(key), str), time_sort_key(key))):
    ...
    def student_view(self, context=None):  # pylint: disable=unused-argument
        ...
        config = config.format(
            video_id=_json_for_inline_script(self.video_id),
            timemap=_json_for_inline_script(
                parse_timemap_field(self.timemap)
            ),
            jump_back=_json_for_inline_script(
                parse_jump_back_field(self.jump_back)
            ),
        )
